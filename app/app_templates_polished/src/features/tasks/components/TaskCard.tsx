...
function TaskCard({ task, onTaskChange, compact = false }: TaskCardProps) {
  ...
  const templateTitle =
    task.sourceTemplateTitle ||
    processTemplates.find((template) => template.id === task.sourceTemplateId)
      ?.title ||
    "";
  const [isCompleting, setIsCompleting] = useState(false);
  const [timeSpentMinutes, setTimeSpentMinutes] = useState(
    String(task.timeSpentMinutes ?? ""),
  );
  const [materialCost, setMaterialCost] = useState(
    String(task.materialCost ?? 0),
  );
  ...
  const handleSkipStep = () => {
    ...
    onTaskChange(
      withTaskUpdate(task, {
        ...
        materialDescription:
          task.materialDescription || "Korak preskočen od strane admina.",
  ...
  const assigneeLabel =
    task.assignedToLabel || assignedUser?.name || "Nije dodeljeno";
  const compactMeta = [
    requiredRole || task.assignedToLabel || "",
    assigneeLabel,
    `Rok ${formatDueDate(task.dueDate)}`,
    task.estimatedMinutes
      ? `Procena ${formatDurationMinutes(task.estimatedMinutes)}`
      : "",
  ]
    .filter(Boolean)
    .join(" · ");
  const billingLabel =
    task.billingState === "ready_for_billing" ||
    task.billingState === "sent_to_billing"
      ? "Naplativo"
      : "Nije naplativo";
    ...
    return (
      <article className="customer-task-card customer-task-card-compact">
        <div className="customer-task-compact-main">
          <div>
            <strong>
              {task.sequenceOrder ? `${task.sequenceOrder}. ` : ""}
              {task.title}
            </strong>
            <p>{compactMeta}</p>
            <span className="customer-billing-state-note">{billingLabel}</span>
            {blockReason ? (
              <p className="customer-source-note">
                🚫 <strong>BLOKIRANO:</strong> {blockReason}
              </p>
            ) : null}
          </div>
        ...
        {task.needsAssignment ? (
          <p className="customer-source-note task-needs-assignment-note">
            ⚠ Potrebna je ručna dodela role pre rada.
          </p>
        ) : null}
        ...
        {isAssigning ? (
          <div className="customer-task-complete-form">
              ...
              >
                ...
                {users.map((user) => (
                  <option key={user.id} value={user.id}>
                    {user.name}{" "}
                    {user.productionRole
                      ? `(${normalizeOperationalRole(user.productionRole)})`
                      : "(bez operativne role)"}
                  </option>
                ))}
              </select>
            ...
            {requiredRole ? (
              <p className="customer-source-note">
                Potrebna rola: <strong>{requiredRole}</strong>
              </p>
            ) : null}
  ...
  return (
    <article className="customer-task-card">
      ...
      {task.source === "template" || templateTitle ? (
        <p className="customer-source-note">
          Iz šablona: <strong>{templateTitle || "Proces"}</strong>
          {task.sourceProductTitle
            ? ` · Proizvod: ${task.sourceProductTitle}`
            : ""}
        </p>
      ) : null}
