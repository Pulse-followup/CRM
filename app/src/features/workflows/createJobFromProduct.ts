...
export function calculateTemplateEstimateMinutes(
  template: ProcessTemplate | null | undefined,
) {
  return (template?.steps || []).reduce(
    (sum, step) => sum + (Number(step.estimatedMinutes) || 0),
    0,
  );
}

export function calculateTemplateEstimateDays(
  template: ProcessTemplate | null | undefined,
) {
  const totalMinutes = calculateTemplateEstimateMinutes(template);
  return totalMinutes > 0
    ? Math.max(1, Math.ceil(totalMinutes / WORKDAY_MINUTES))
    : 0;
}

export function formatTemplateEstimate(
  template: ProcessTemplate | null | undefined,
) {
  const totalMinutes = calculateTemplateEstimateMinutes(template);
...
function calculateStepDueDate(
  ...
  const days =
    cumulativeMinutes > 0
      ? Math.max(1, Math.ceil(cumulativeMinutes / WORKDAY_MINUTES))
      : 0;
...
function isAdminMember(member: CatalogJobTeamMember) {
  return (
    normalizeRoleLabel(member.productionRole || "") === "ADMIN" ||
    member.name.toLowerCase().includes("admin")
  );
}

function findMemberByProductionRole(
  role: string,
  members: CatalogJobTeamMember[] = [],
) {
  const normalizedRole = normalizeRoleLabel(role);
  const activeMembers = members.filter((member) => member.id);

  const exactMatch = activeMembers.find(
    (member) =>
      normalizeRoleLabel(member.productionRole || "") === normalizedRole,
  );
  ...
  const adminMatch =
    activeMembers.find(isAdminMember) || activeMembers[0] || null;
  return { member: adminMatch, needsAssignment: true };
}

function buildTaskDescription(draft: CatalogJobDraft, stepTitle: string) {
  const lines = [
    `${stepTitle} za proizvod: ${draft.product.title}${draft.quantity ? ` · količina: ${draft.quantity}` : ""}.`,
  ];
...
export function buildCatalogJobPayload(
  draft: CatalogJobDraft,
  teamMembers: CatalogJobTeamMember[] = [],
): CatalogJobPayload {
  const timestamp = new Date().toISOString();
  const projectId = safeId("project");
  const sortedSteps = [...draft.template.steps].sort(
    (firstStep, secondStep) => firstStep.order - secondStep.order,
  );
  ...
  const project: Project = {
    ...
    dueDate:
      draft.dueDate || calculateDefaultDueDate(draft.template) || undefined,
  ...
  const tasks: Task[] = sortedSteps.map((step, index) => {
    ...
    return {
      ...
      assignedToLabel: assignment.needsAssignment
        ? `${assignmentLabel} · potrebna dodela (${requiredRole})`
        : assignmentLabel,
      requiredRole,
      needsAssignment: assignment.needsAssignment,
      dueDate:
        calculateStepDueDate(draft.template, index) ||
        draft.dueDate ||
        undefined,
